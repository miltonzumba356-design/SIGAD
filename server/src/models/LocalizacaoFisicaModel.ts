...
export class LocalizacaoFisicaModel {
  ...
  static softDelete(id: number, deletadoPor: number): boolean {
    ...
    // Verificar se há documentos associados a esta localização
    const documentos = db.prepare(`
      SELECT COUNT(*) as count
      FROM documentos_localizacao dl
      JOIN documentos d ON d.id = dl.documento_id
      WHERE dl.localizacao_id = ? AND d.deleted_at IS NULL
    `).get(id) as { count: number };
    if (documentos.count > 0) {
      throw new Error('LOCATION_HAS_DOCUMENTS');
    }
