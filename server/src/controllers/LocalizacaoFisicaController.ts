...
export class LocalizacaoFisicaController {
  ...
  static criar(req: AuthRequest, res: Response) {
    try {
      ...
      if (!nome || !tipo) {
        return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Nome e tipo são obrigatórios.' } });
      }

      if (!['EDIFICIO', 'SALA', 'ESTANTE', 'PRATELEIRA', 'CAIXA'].includes(tipo)) {
        return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Tipo de localização inválido.' } });
      }

      if (parent_id) {
        const parent = LocalizacaoFisicaModel.buscarPorId(Number(parent_id));
        if (!parent || parent.instituicao_id !== instituicaoId) {
          return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Localização superior não encontrada.' } });
        }
      }
  ...
  static excluir(req: AuthRequest, res: Response) {
    try {
      ...
      if (error.message === 'LOCATION_HAS_DOCUMENTS') {
        return res.status(409).json({ error: { code: 'CONFLICT', message: 'Localização com documentos associados não pode ser eliminada.' } });
      }
