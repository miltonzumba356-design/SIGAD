...
export class FilaDigitalizacaoController {
  ...
  static adicionar(req: AuthRequest, res: Response) {
    try {
      ...
      if (!documentId) {
        return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Documento é obrigatório.' } });
      }
      ...
      if (tipoFila === 'FISICA' && !localizacaoId) {
        return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Fila física exige localização física.' } });
      }
      ...
      if (localizacaoId) {
        ...
        if (!localizacao || localizacao.instituicao_id !== req.usuario!.instituicao_id) {
          return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Localização física não encontrada.' } });
        }
      }
      ...
      res.status(201).json({ data: { id, message: 'Documento adicionado à fila de digitalização.' } });
    } catch (error) {
  ...
  static assumir(req: AuthRequest, res: Response) {
    try {
      ...
      if (sucesso) {
        ...
        res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Item não encontrado ou já em progresso.' } });
      }
    } catch (error) {
  ...
  static concluir(req: AuthRequest, res: Response) {
    try {
      ...
      if (resultado.sucesso && resultado.documento_id) {
        ...
        res.json({ data: { message: 'Digitalização concluída com sucesso.' } });
      } else {
        res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Item não encontrado.' } });
      }
    } catch (error) {
  ...
  static async digitalizarScanner(req: AuthRequest, res: Response) {
    try {
      ...
      if (!scanner_ip || !title) {
        return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'IP do scanner e título são obrigatórios.' } });
      }
      ...
      if ((suporte === 'AMBOS') && !localizacaoId) {
        return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Documento híbrido deve indicar localização física.' } });
      }
      ...
      if (duplicate) {
        ...
        return res.json({
          data: {
            ...
            message: 'Possível documento duplicado encontrado.'
          }
        });
      }
      ...
      res.status(502).json({
        error: {
          ...
          message: 'Não foi possível acionar o scanner. Confirme o IP, se o equipamento suporta eSCL/AirScan e se está na mesma rede.'
        }
      });
    }
  }
  ...
  static resolverDuplicidade(req: AuthRequest, res: Response) {
    try {
      ...
      if (!['KEEP', 'SUBSTITUTE', 'IGNORE'].includes(action)) {
        return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Ação inválida.' } });
      }
      ...
      if (!pending) {
        return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Digitalização pendente não encontrada.' } });
      }
      ...
      if (action === 'IGNORE') {
        ...
        return res.json({ data: { ignored: true, message: 'Digitalização ignorada.' } });
      }
      ...
      if (!targetDocumentId) {
        return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Documento original não encontrado para substituição.' } });
      }
      ...
      if (action === 'SUBSTITUTE') {
        ...
        if (!existing || existing.instituicao_id !== req.usuario!.instituicao_id) {
          return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Documento original não encontrado.' } });
        }
      ...
      res.json({
        data: {
          ...
          action,
          message: action === 'SUBSTITUTE' ? 'Documento substituído com sucesso.' : 'Documento mantido como novo registo.'
        }
      });
    } catch {
