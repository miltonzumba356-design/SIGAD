...
export class DocumentoController {
  ...
  static async criar(req: AuthRequest, res: Response) {
    try {
      ...
      if (!title) {
        return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Título é obrigatório.' } });
      }
      ...
      if ((suporte === 'FISICO' || suporte === 'AMBOS') && !localizacaoId) {
        return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Documentos físicos devem indicar a localização.' } });
      }
  ...
  static async atualizar(req: AuthRequest, res: Response) {
    try {
      ...
      if (!sucesso) {
        return res.status(400).json({ error: { code: 'UPDATE_FAILED', message: 'Nenhuma alteração realizada.' } });
      }
  ...
  static excluir(req: AuthRequest, res: Response) {
    try {
      ...
      if (sucesso) {
        ...
        res.status(400).json({ error: { code: 'DELETE_FAILED', message: 'Não foi possível excluir o documento.' } });
      }
    } catch (error: any) {
      if (error.message === 'DOCUMENT_HAS_ACTIVE_LOAN') {
        return res.status(409).json({ error: { code: 'CONFLICT', message: 'Documento com empréstimo ativo não pode ser eliminado.' } });
      }
