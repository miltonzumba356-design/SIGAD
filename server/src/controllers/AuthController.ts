...
export class AuthController {
  ...
  static async refresh(req: Request, res: Response) {
    ...
    if (!refresh_token) {
      return res.status(400).json({ error: { code: 'BAD_REQUEST', message: 'Refresh token obrigatório.' } });
    }
    ...
    try {
      ...
      if (!usuario || !usuario.ativo) {
        return res.status(401).json({ error: { code: 'UNAUTHORIZED', message: 'Usuário inválido.' } });
      }
  ...
  static async switchInstitution(req: AuthRequest, res: Response) {
    try {
      ...
      if (!instituicaoId) {
        return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Instituição obrigatória.' } });
      }
      ...
      if (!canSwitch && usuario.instituicao_id !== instituicaoId) {
        return res.status(403).json({ error: { code: 'PERMISSION_DENIED', message: 'Sem permissão para trocar de instituição.' } });
      }
      ...
      res.status(500).json({ error: { code: 'INTERNAL_ERROR', message: 'Erro ao trocar instituição.' } });
    }
  }
}
