...
function ensureSchemaCompatibility(database: SqliteDatabase): void {
  ...
  try {
    if (hasTable(database, 'permissoes') && hasTable(database, 'role_permissoes') && hasTable(database, 'roles')) {
      const allPerms = database.prepare("SELECT id FROM permissoes").all() as { id: number }[];
      const adminRoles = database.prepare("SELECT id FROM roles WHERE nome IN ('Super Admin', 'Admin Institucional')").all() as { id: number }[];
...
function ensureCorePermissions(database: SqliteDatabase): void {
  ...
  if (!hasTable(database, 'role_permissoes') || !hasTable(database, 'roles')) return;

  const insertRolePermissaoPorNome = database.prepare(`
    INSERT OR IGNORE INTO role_permissoes (role_id, permissao_id)
    SELECT r.id, p.id FROM roles r, permissoes p
    WHERE r.nome = ? AND p.codigo = ?
  `);

  for (const codigo of ['dig.physical', 'users.deactivate', 'audit.view']) {
    insertRolePermissaoPorNome.run('Super Admin', codigo);
    insertRolePermissaoPorNome.run('Admin Institucional', codigo);
  }
  insertRolePermissaoPorNome.run('Digitalizador', 'dig.physical');
}
