...
export function verificarAutenticacao(
  ...
  try {
    ...
    if (!authHeader) {
      res.status(401).json({
        error: { code: 'UNAUTHORIZED', message: 'Token de autenticação não fornecido.' }
      });
      return;
    }
...
export function temPermissao(permissaoRequerida: string) {
  return (req: AuthRequest, res: Response, next: NextFunction): void => {
    ...
    if (!req.usuario.permissoes.includes(permissaoRequerida)) {
      ...
      res.status(403).json({ error: { code: 'PERMISSION_DENIED', message: 'Sem permissão para esta operação.' } });
      return;
    }
