...
const navItems: { section: string; items: NavItemProps[] }[] = [
  {
    ...
    items: [
      ...
      { id: 'fisico', label: 'Arquivo Físico', icon: <Archive size={18} />, path: '/arquivo-fisico' },
    ]
  },
  {
    section: 'Operações',
    items: [
      { id: 'emprestimos', label: 'Empréstimos', icon: <ClipboardList size={18} />, path: '/emprestimos' },
      { id: 'pesquisa', label: 'Pesquisa Avançada', icon: <Search size={18} />, path: '/pesquisa' },
      { id: 'digitalizacao', label: 'Digitalização', icon: <Printer size={18} />, path: '/digitalizacao' },
    ]
  },
  {
    section: 'Gestão',
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const switchInstitution = async (instituicaoId: number) => {
    ...
    if (res.data) {
      ...
      setUser(nextUser);
      toast.success('Instituição alterada');
      window.location.reload();
    } else {
      toast.error(res.error?.message || 'Erro ao trocar instituição');
    }
  };
  ...
  return (
    <aside className={`sidebar ${isOpen ? 'open' : ''}`}>
      <div className="logo">
        ...
        <h1>Gestão de Arquivos e Documentos</h1>
        ...
        {canSwitch && institutionMenuOpen && (
          ...
          >
            ...
            {instituicoes.length === 0 && (
              <div style={{ padding: 10, color: 'var(--muted)', fontSize: 11 }}>Nenhuma instituição disponível</div>
            )}
          </div>
        )}
      </div>
      ...
      <div className="sidebar-footer" style={{ borderTop: '1px solid #27272a', paddingTop: '16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
          ...
          <div className="user-info">
            <div className="user-name">Sessão ativa</div>
