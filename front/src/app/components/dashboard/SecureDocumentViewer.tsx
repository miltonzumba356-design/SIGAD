...
export function SecureDocumentViewer({ document, isOpen, onClose }: SecureDocumentViewerProps) {
  ...
  React.useEffect(() => {
    ...
    Promise.all([
      ...
      if (blobRes.error && textRes.error) toast.error('Não foi possível abrir o documento');
      ...
      <div className="secure-viewer">
        <div className="secure-viewer-header">
          <div>
            ...
            <div className="secure-viewer-meta">Visualização protegida - sem download, sem impressão, com marca de utilizador</div>
          </div>
        ...
        <div className="secure-viewer-body">
          ...
          {!loading && (!fileUrl || (!isPdf && !isImage)) && (
            <div className="secure-text">
              <FileText size={18} />
              <pre>{text || 'Sem texto extraído para leitura.'}</pre>
            </div>
          )}
        </div>
      </div>
