...
export function PesquisaAvancada() {
  ...
  const pesquisar = async () => {
    ...
    else toast.error(res.error?.message || 'Erro ao pesquisar no índice OCR');
  };
  ...
  const indexarDocumentos = async () => {
    ...
    setIndexing(false);
    toast.success(`Indexação concluída: ${ok} documentos indexados${fail ? `, ${fail} sem ficheiro digital` : ''}`);
    if (query.trim()) pesquisar();
  };
  ...
  return (
    <Layout title="Pesquisa Avançada" subtitle="Pesquisa por texto digital e OCR em documentos indexados">
      <div className="view active">
        ...
        <div className="card">
          <div className="card-title">
            Resultados - <span style={{ color: 'var(--accent)', fontSize: 13 }}>{results.length} ocorrências</span>
          ...
          {loading ? (
              ...
              {results.map(result => (
                <div className="search-result" key={result.docId}>
                  ...
                  <div className="search-result-body">
                    ...
                    <div className="search-result-snippet">{renderSnippet(result.snippet || 'Sem excerto disponível.')}</div>
                    <div className="search-result-meta">
                      Score {result.score}% - {result.matchCount} ocorrências - {result.pages} página(s)
                    </div>
                  </div>
            ...
            <div style={{ textAlign: 'center', padding: 40, color: 'var(--muted)' }}>
              {query ? 'Nenhum documento encontrado no índice.' : 'Digite uma palavra ou frase para pesquisar no OCR.'}
            </div>
          )}
        </div>
      </div>
