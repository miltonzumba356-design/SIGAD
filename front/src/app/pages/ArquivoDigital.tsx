...
export function ArquivoDigital() {
  ...
  const deleteFolder = async (e: React.MouseEvent, folder: any) => {
    e.stopPropagation();
    if (!window.confirm(`Eliminar a pasta "${folder.nome}"? Esta ação não pode ser desfeita.`)) return;
    const res = await api.request(`/folders/${folder.id}`, { method: 'DELETE' });
    if (res.data) {
      toast.success('Pasta eliminada com sucesso');
      loadData(currentFolder?.id || null);
    } else {
      toast.error(res.error?.message || 'Erro ao eliminar pasta');
    }
  };
  ...
  return (
    <Layout title="Arquivo Digital" subtitle="Gestão de documentos e pastas">
      <div className="view active">
        ...
        <div className="folder-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px', marginBottom: '32px' }}>
          ...
          {folders.map(folder => (
            <div key={folder.id} className="folder-card" style={{ position: 'relative' }} onClick={() => enterFolder(folder)}>
              <div style={{ position: 'absolute', top: 8, right: 8, display: 'flex', gap: 4 }}>
                <button className="btn btn-ghost btn-sm" title="Editar pasta" onClick={(e) => openEditFolder(e, folder)}><Pencil size={13} /></button>
                <button className="btn btn-ghost btn-sm" title="Eliminar pasta" onClick={(e) => deleteFolder(e, folder)}><Trash2 size={13} /></button>
              </div>
              <div className="folder-icon"><Folder size={28} fill="currentColor" opacity={0.2} /></div>
              <div className="folder-name">{folder.nome}</div>
              <div className="folder-count">{folder.codigo || 'Sem código'}</div>
            </div>
          ))}
        </div>
        ...
        <div className="card">
          <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <div className="card-title" style={{ marginBottom: 0 }}>
              {currentFolder ? `Conteúdo de: ${currentFolder.nome}` : 'Documentos na Raiz'}
            </div>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table>
                ...
                <tbody>
                  ...
                  {documents.length === 0 && (
                    <tr><td colSpan={6} style={{ textAlign: 'center', padding: '40px', color: 'var(--muted)' }}>Esta pasta não contém documentos.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
