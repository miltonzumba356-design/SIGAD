...
export function Instituicoes() {
  ...
  async function loadData() {
    ...
    if (res.data) {
      ...
      setError(res.error?.message || 'Sem permissão para listar instituições.');
    }
    setLoading(false);
  }
  ...
  return (
    <Layout title="Instituições" subtitle="Plataforma multi-institucional">
      <div className="view active">
        <div className="card">
          ...
          <div style={{ overflowX: 'auto' }}>
            <table>
              <thead><tr><th>Instituição</th><th>Código</th><th>Armazenamento</th><th>Estado</th></tr></thead>
              <tbody>
                {loading ? (
                ...
                {!loading && instituicoes.length === 0 && (
                  <tr><td colSpan={4} style={{ textAlign: 'center', padding: '40px', color: 'var(--muted)' }}>{error || 'Nenhuma instituição cadastrada.'}</td></tr>
                )}
              </tbody>
            </table>
          </div>
