...
export function Digitalizacao() {
  ...
  const loadData = async (filter = queueFilter) => {
    ...
    else toast.error(filaRes.error?.message || 'Erro ao carregar fila de digitalização');
  ...
  const digitalizar = async () => {
    if (!form.scanner_ip.trim() || !form.title.trim()) {
      toast.error('Informe o IP do scanner e o título do documento');
      return;
    }
    ...
    if (form.support === 'AMBOS' && !form.localizacao_id) {
      toast.error('Documento híbrido deve indicar a localização física');
      return;
    }
  ...
  const resolveDuplicate = async (action: 'KEEP' | 'SUBSTITUTE' | 'IGNORE') => {
    ...
    if (res.data) {
      const messages = {
        ...
        SUBSTITUTE: 'Documento existente substituído',
        IGNORE: 'Digitalização ignorada'
      };
  ...
  const concluir = async () => {
    ...
    if (res.data) {
      toast.success('Digitalização concluída');
      closeComplete();
      await loadData();
    } else {
      toast.error(res.error?.message || 'Erro ao concluir digitalização');
    }
    setCompleting(false);
  };

  return (
    <Layout title="Digitalização" subtitle="Scanner de rede, fila de digitalização e OCR">
      <div className="view active">
        <div className="kpi-grid">
          ...
          <div className="kpi-card">
            ...
            <div className="kpi-label">Fila física</div>
        ...
        <div className="card">
          ...
          <div style={{ overflowX: 'auto' }}>
            <table>
              <thead><tr><th>Código</th><th>Título</th><th>Fila</th><th>Localização</th><th>Prioridade</th><th>Estado</th><th>Ação</th></tr></thead>
              <tbody>
                {loading ? (
                  ...
                  <tr key={item.id}>
                    ...
                    <td><span className="tag tag-publico">{item.tipo_fila === 'FISICA' ? 'Física' : 'Digital'}</span></td>
                ...
                {!loading && fila.length === 0 && (
                  <tr><td colSpan={7} style={{ textAlign: 'center', padding: '40px', color: 'var(--muted)' }}>Nenhum documento na fila de digitalização.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="modal" style={{ maxWidth: 780 }}>
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <input className="form-input" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Ex: Declaração de serviço" />
              </div>
            </div>
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <select className="form-input" value={form.type} onChange={e => setForm({ ...form, type: e.target.value })}>
                  ...
                  <option value="Despacho Ministerial">Despacho Ministerial</option>
                  <option value="Nota Interna">Nota Interna</option>
                  <option value="Contrato de Concessão">Contrato de Concessão</option>
                  <option value="Processo de Licenciamento">Processo de Licenciamento</option>
                  ...
                  <option value="Correspondência Externa">Correspondência Externa</option>
                  <option value="Relatório de Auditoria Interna">Relatório de Auditoria Interna</option>
                  <option value="Processo de Concessão">Processo de Concessão</option>
                </select>
              </div>
              <div className="form-group">
                ...
                <select className="form-input" value={form.support} onChange={e => setForm({ ...form, support: e.target.value as Support })}>
                  <option value="AMBOS">Físico + Digitalizado</option>
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                <label className="form-label"><Archive size={12} /> Local físico</label>
                <select className="form-input" value={form.localizacao_id} onChange={e => setForm({ ...form, localizacao_id: e.target.value })}>
                  <option value="">Selecionar localização física</option>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 16 }}>
              <div className="card" style={{ margin: 0 }}>
                ...
                <div style={{ display: 'grid', gap: 8, fontSize: 12 }}>
                  <strong>{duplicateWarning.existing_document?.titulo || 'Sem título'}</strong>
                  <span>Tipo: {duplicateWarning.existing_document?.tipo || '-'}</span>
                  <span>Suporte: {duplicateWarning.existing_document?.suporte || '-'}</span>
                  <span>Classificação: {duplicateWarning.existing_document?.classificacao || '-'}</span>
              ...
              <div className="card" style={{ margin: 0 }}>
                <div className="card-title">Nova digitalização</div>
                <div style={{ display: 'grid', gap: 8, fontSize: 12 }}>
                  ...
                  <span>Classificação: {duplicateWarning.scanned_document.classification || '-'}</span>
