...
export function ArquivoFisico() {
  ...
  const loadTree = async () => {
    ...
    if (res.data) {
      ...
      toast.error(res.error?.message || 'Erro ao carregar arquivo físico');
    }
    setLoading(false);
  };
  ...
  const handleCreateLocation = async () => {
    ...
    if (res.data) {
      ...
      toast.error(res.error?.message || 'Erro ao criar localização');
    }
  };
  ...
  const handleUpdateLocation = async () => {
    ...
    if (res.data) {
      toast.success('Nome da localização atualizado');
      setEditingLocation(false);
      await loadTree();
    } else {
      toast.error(res.error?.message || 'Erro ao atualizar localização');
    }
  };

  const handleDeleteLocation = async () => {
    if (!selectedLocation) return;
    if (!window.confirm(`Eliminar "${selectedLocation.nome}"? Esta ação não pode ser desfeita.`)) return;

    const res = await api.request(`/physical-archive/${selectedLocation.id}`, { method: 'DELETE' });
    if (res.data) {
      ...
      toast.error(res.error?.message || 'Erro ao eliminar localização');
    }
  };
  ...
  const digitizeDocument = async (doc: DocumentRow) => {
    ...
    setSelectedDocument(res.data || doc);
    setEditOpen(true);
    toast.info('Carregue a versão digitalizada no campo de nova versão e guarde.');
  };
  ...
  const getConditionLabel = (condition?: string) => {
    ...
    if (condition === 'FAIR') return 'Razoável';
    return 'Bom';
  };
  ...
  return (
    <Layout title="Arquivo Físico" subtitle="Edifícios, salas, estantes, prateleiras e documentos físicos">
      <div className="view active">
        <div className="two-col">
          <div className="card">
            <div className="section-title"><Building size={16} /> Localização Física</div>
            {loading ? (
              ...
              <div style={{ color: 'var(--muted)', fontSize: '12px' }}>Nenhuma localização cadastrada.</div>
            )}
          </div>

          <div className="card">
            <div className="section-title"><Plus size={16} /> Nova Localização</div>
            ...
            <div className="form-group">
              ...
              <input className="form-input" value={formData.nome} onChange={e => setFormData({ ...formData, nome: e.target.value })} placeholder="Ex: Edifício Central, Sala A, Estante 01" />
            </div>
            <div className="form-row">
              <div className="form-group">
        ...
        <div className="card">
          <div className="card-title">
            Documentos em: {selectedLocation?.nome || 'selecione uma localização'}
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {selectedLocation && (
                <>
                  <span onClick={() => setEditingLocation(true)} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Pencil size={14} /> Editar Localização
                  </span>
                  <span onClick={handleDeleteLocation} style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--red)' }}>
                ...
                {!docsLoading && documents.length === 0 && (
                  <tr>
                    <td colSpan={7} style={{ textAlign: 'center', padding: '40px', color: 'var(--muted)' }}>
                      Nenhum documento registado nesta localização.
                    </td>
                  </tr>
                )}
              </tbody>
